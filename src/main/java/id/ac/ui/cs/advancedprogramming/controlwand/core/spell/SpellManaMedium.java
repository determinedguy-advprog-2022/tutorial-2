...
// Class Command
public class SpellManaMedium implements Spell {

    MagicTool magicTool;

    public SpellManaMedium(MagicTool magicTool) {
        this.magicTool = magicTool;
    }

    @Override
    public void cast() {
        if (!this.magicTool.getCurrentState().equals(ManaIntensity.MEDIUM)) {
            this.magicTool.medium();
            this.magicTool.addToStateForUndo(ManaIntensity.MEDIUM);
        }
    }
    ...
    public String spellName() {
        return String.format("%s:MEDIUM", this.magicTool.getName());
    }
}
