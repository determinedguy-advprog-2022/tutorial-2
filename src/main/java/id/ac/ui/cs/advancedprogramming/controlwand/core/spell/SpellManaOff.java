...
// Class Command
public class SpellManaOff implements Spell {

    MagicTool magicTool;

    public SpellManaOff(MagicTool magicTool) {
        this.magicTool = magicTool;
    }

    @Override
    public void cast() {
        if (!this.magicTool.getCurrentState().equals(ManaIntensity.NONE)) {
            this.magicTool.off();
            this.magicTool.addToStateForUndo(ManaIntensity.NONE);
        }
    }
    ...
    public String spellName() {
        return String.format("%s:NONE", this.magicTool.getName());
    }
}
